  ...
  def eu_vou evento, nome = "Eu vou!", callback = "Confirmado!", st = '', tipo = 'eu_vou'
    ...
    if user_signed_in?
      if current_user.tem_gadget? evento,tipo
        ...
        content << link_to( nome, gadgets_path(:evento => evento.id, :tipo => Gadget.busca_por_tipo(tipo)[:nome]), :class => 'ajax_vou ' + st, :id => "ajax_vou_#{evento.id}", :remote => true     )   
      end  
    else
      content << link_to( nome, gadgets_path(:evento => evento.id, :tipo => Gadget.busca_por_tipo(tipo)[:nome]), :class => 'ajax_vou ' + st, :id => "ajax_vou_#{evento.id}"    )   
    end
